      ...
      <div className="calendar-header">
        <div className="header-center">
          <span className="prev-month">
            <span onClick={this.preMonth}>{leftArrow}</span>
          </span>
          <span className="title">
            {this.state.currentYear}年{this.state.currentMonth}月
          </span>
          <span className="next-month">
            <span onClick={this.nextMonth}>{rightArrow}</span>
          </span>
        </div>
      </div>
    );
      ...
      <div className="calendar-body">
        <div className="week-container">
          {this.state.weekList.map((week) => {
            return (
              <div key={week.name} className={`week ${week.className}`}>
                {week.name}
              </div>
            );
          })}
        ...
        <div className="day-container">
          {this.state.dayList.map((dayObject, index) => {
            return (
              <div key={index} className={`day ${dayObject.className}`}>
                {dayObject.day}
              </div>
            );
          })}
